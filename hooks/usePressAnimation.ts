...
export function usePressAnimation(configKey: PressType, disabled = false, lifted = false) {
  ...
  // Drag-lift. This hook is now the ONLY writer of outer-layer shadow + border.
  useEffect(() => {
    const target = lifted ? LIFT_DRAG : LIFT_REST;
    const timing = { duration: reduceMotion ? 0 : lifted ? 150 : LIFT_OUT_MS, easing: LIFT_EASING };
    cancelAll();
    shadowRadius.value  = withTiming(target.shadowRadius,  timing);
    shadowOpacity.value = withTiming(target.shadowOpacity, timing);
    shadowOffsetY.value = withTiming(target.shadowOffsetY, timing);
    liftElevation.value = withTiming(target.elevation,     timing);
  ...
  const onPressOut = useCallback(() => {
    if (disabled || reduceMotion) {
      ...
      scale.value = 1;
      const target = lifted ? LIFT_DRAG : LIFT_REST;
      shadowRadius.value = target.shadowRadius;
      shadowOpacity.value = target.shadowOpacity;
      shadowOffsetY.value = target.shadowOffsetY;
      liftElevation.value = target.elevation;
      borderOpacity.value = target.borderOpacity;
      return;
    }
